...
# Function to convert words to sentence, separated by "\n" in the dataset.
...
# Extracting all the predicted diseases
...
for key in disease_treatment.keys():
    if disease_treatment[key] != []:
        final_dict[key] = disease_treatment[key]
...
import pandas as pd 
df = pd.DataFrame({"Disease":final_dict.keys(),"Treatments":final_dict.values()})
df.head()
...
treatments = final_dict[search_item]
